...
export const commands = {
  ...
  async isSupportedLanguagesLive(
    ...
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:listener|is_supported_languages_live", { provider, model, languages, },),
      };
    } catch (e) {
      if (e instanceof Error) throw e;
      else return { status: "error", error: e as any, };
  ...
  async suggestProvidersForLanguagesLive(languages: string[],): Promise<Result<string[], string>> {
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:listener|suggest_providers_for_languages_live", { languages, },),
      };
    } catch (e) {
      if (e instanceof Error) throw e;
      else return { status: "error", error: e as any, };
...
export type SessionDataEvent = { type: "audio_amplitude"; session_id: string; mic: number; speaker: number; } | {
  type: "mic_muted";
  session_id: string;
  value: boolean;
...
export type SessionLifecycleEvent = { type: "inactive"; session_id: string; error: string | null; } | {
  type: "active";
  session_id: string;
...
export type StreamResponse =
  ...
  | { type: "Metadata"; request_id: string; created: string; duration: number; channels: number; }
