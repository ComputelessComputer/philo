...
export const commands = {
  ...
  async saveSessionTranscript(sessionId: string, transcript: TranscriptData,): Promise<Result<null, string>> {
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:fs-db|save_session_transcript", { sessionId, transcript, },),
      };
    } catch (e) {
      if (e instanceof Error) throw e;
      else return { status: "error", error: e as any, };
  ...
  async saveSessionEnhancedNote(
    ...
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:fs-db|save_session_enhanced_note", { sessionId, note, filename, },),
      };
    } catch (e) {
      if (e instanceof Error) throw e;
      else return { status: "error", error: e as any, };
